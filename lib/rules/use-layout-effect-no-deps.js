/**
 * @fileoverview Warns a user when they call the react useLayoutEffect hook without a dependency array
 * @author Alex Henry
 */
"use strict";
...
module.exports = {
  meta: {
    docs: {
      description:
        "Warns a user when they call the react useLayoutEffect hook without a dependency array",
  ...
  create: function(context) {
    ...
    function visitFunctionExpression(node) {
      ...
      if (reactiveHookName !== "useLayoutEffect") {
        return;
      }

      // Get the declared dependencies for this reactive hook. If there is no
      // second argument then the useLayoutEffect callback will re-run on every render.
      ...
      if (!declaredDependenciesNode) {
        context.report({
          ...
          message:
            "useLayoutEffect re-runs on every render when called with one argument. Did you forget to pass a dependencies array?"
        });
      }
    }
